...
### 9/16/2020
1. Fix `excludedPath` not working when is not exact match like `/foo/*`
2. Remove the unneeded preload link in the AMP which generated by Gatsby.
3. Remove all `!important` in custom style as it is illegal for AMP

as the original author seems no longer maintain it, so here released a new package.

## Install

`npm install --save gatsby-plugin-amp-fork`
